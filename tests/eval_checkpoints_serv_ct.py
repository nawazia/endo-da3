"""
Evaluate all stage checkpoints on SERV-CT to track progression.

Usage:
    python tests/eval_checkpoints_serv_ct.py
"""
...
from tools.eval_serv_ct import evaluate

GASTRONET   = "/home/in4218/code/GastroNet/gastronet/dinov2.pth"
SERV_CT     = "/home/in4218/code/data/SERV-CT"
IMG_SIZE    = 336
...
def main():
    ...
    print(f"{'Checkpoint':<22} {'AbsRel':>8} {'δ<1.25':>8} {'RMSE(mm)':>10}")
    print("-" * 52)

    # DA3-BASE baseline (no GastroNet, no fine-tuning)
    model = load_da3_base(device)
    m = evaluate(model, SERV_CT, img_size=IMG_SIZE, device=device)
    print(f"{'DA3-BASE':<22} {m['AbsRel']:>8.4f} {m['d1']:>8.4f} {m['RMSE']*1000:>10.1f}")
    del model
    ...
    for label, ckpt_path, use_lora in CHECKPOINTS:
        if not Path(ckpt_path).exists():
            print(f"{label:<22}  (not found: {ckpt_path})")
            continue
        model = load_model(ckpt_path, use_lora, device)
        m = evaluate(model, SERV_CT, img_size=IMG_SIZE, device=device)
        print(f"{label:<22} {m['AbsRel']:>8.4f} {m['d1']:>8.4f} {m['RMSE']*1000:>10.1f}")
        del model
        torch.cuda.empty_cache()
