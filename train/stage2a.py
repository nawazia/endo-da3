"""
Stage 2a — Real-domain alignment on Hamlyn + StereoMIS P1.
...
Data:
    Train : Hamlyn daVinci (34,240 pairs) + StereoMIS P1 (18,420 pairs)
    Val   : Hamlyn daVinci val split (last 10% of test set)

Initialisation: Stage 1 best.pt

Run:
    python train/stage2a.py \\
        --stage1-ckpt runs/stage1/best.pt \\
        --gastronet ~/code/GastroNet/gastronet/dinov2.pth \\
        --hamlyn-root ~/code/data/Hamlyn/daVinci \\
        --stereomis-root ~/code/data/StereoMIS \\
...
from endo_da3.data.loaders import make_stage2a_loaders
...
def train(args):
    ...
    train_loader, val_loader, ds_names = make_stage2a_loaders(
        hamlyn_root=args.hamlyn_root,
        stereomis_root=args.stereomis_root,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_depth_l1 < best_val:
            ...
            print(f"  ★ new best val depth_l1: {best_val*1000:.1f}mm")


# ── CLI ────────────────────────────────────────────────────────────────────────

def parse_args():
    p = argparse.ArgumentParser("Stage 2a: real-domain alignment")
